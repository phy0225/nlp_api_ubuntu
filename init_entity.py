...
def Dictest(line, view_dict_set, keywords):
    ...
    fa = open(aresult, 'w', encoding='utf-8')  # 文本的命名实体识别结果
    word = []
    dic = []
    for w in line:
        word.append(w)
...
def crf_prodict():
    ...
    # model = 'model/Dic_model'
    # # 进行测试
    # test = 'temp/test_Dic'
    # output = 'temp/test_Dic_Result'
    # # tagger = CRFPP.Tagger("/opt/CRF++-0.58/crf_test"+" -m " + model)
    # # crf_segmenter(test, output, tagger)
    # try:
    #     process = subprocess.Popen("crf_test/" + model + ' ' + test + ' > ' + output, shell=True)
...
def init(text):
    try:
        fVar = open('dict/symptom.txt', 'r', encoding='utf8')
        keywords = []
...
if __name__ == '__main__':
   ...
   text1 = '患者腹胀较前减轻，咳嗽，咳痰，量少色白，质稀，发热，乏力好转，胃纳一般，二便调，' \
