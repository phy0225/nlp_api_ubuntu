...
def build_test_data_from_crf(text):
    ...
    for json_dict in json_list:
        ...
        indices = json_dict['View']  # 症状位置
        for index in indices[::-1]:  # 倒序搜索
            if len(index) == 1:
                print(index)
                start, end = 0, index[0]
            else:
        ...
        words_tags = cut_sentence(sentence_correct, string=True)  # 结巴分词
    ...
    for key in rjs.keys():
        if rjs[key] not in ['', ' ']:
            djs[key] = rjs[key]
    djs = sorted(djs.items(), key=lambda f: f[1], reverse=True)
    # 转化为 json
    ddjs = []
    for i in djs:
        t = dict()
...
if __name__ == '__main__':
    ...
    tex1 = '患者腹胀较前减轻，咳嗽，咳痰，量少色白，质稀，发热，乏力好转，' \
           '胃纳一般，二便调，夜寐尚可。查体：全身皮肤粘膜轻度黄染，无瘀斑瘀点，' \
           '全身浅表淋巴结未扪及肿大，蜘蛛痣（+），肝掌（＋）。巩膜黄染，腹膨隆，' \
           '全腹无明显压痛、反跳痛、肌卫，肝肋下未及，脾脏肿大平脐，肝肾区叩痛（－），' \
           '麦氏征（-），莫氏征（-），移动性浊音（+），肠鸣音不亢。四肢脊柱无红肿畸形，' \
           '双下肢水肿。患者水肿较前加重，增加利尿剂剂量为速尿40mg qdpo，安体舒通80mg qdpo。' \
           '补液后予拖拉塞米20mg 静推利尿减轻水肿。患者今晨发热38℃，咳嗽，咳痰，量少色白，质稀，' \
           '予NS100ml+海西丁3.0g bid ivgtt抗感染，今补充诊断：肺部感染。患者舌红，苔黄腻，脉弦细予荆银合剂2瓶疏风清热。明晨急查血培养st!。患者尿常规示红细胞++，请肾病科会诊协助诊治。余治同前，继观。'
    
    b = build_test_data_from_crf(tex1)
